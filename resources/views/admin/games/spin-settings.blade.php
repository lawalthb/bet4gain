...
@section('title', 'Spin Game Management')
@section('header', 'Spin Game Management')
...
<div class="space-y-6">
    ...
    <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <div class="bg-white p-6 rounded-lg shadow">
            <h3 class="text-sm font-medium text-gray-500">Total Bets Amount</h3>
            <p class="text-2xl font-bold">₦{{ number_format($statistics['total_bets']) }}</p>
        </div>
        <div class="bg-white p-6 rounded-lg shadow">
            <h3 class="text-sm font-medium text-gray-500">Average Multiplier</h3>
            <p class="text-2xl font-bold">{{ number_format($statistics['average_multiplier'], 2) }}x</p>
        </div>
        <div class="bg-white p-6 rounded-lg shadow">
            <h3 class="text-sm font-medium text-gray-500">Most Common Color</h3>
            <p class="text-2xl font-bold capitalize">{{ $statistics['most_common_color'] }}</p>
        </div>
    </div>
    ...
    <div class="bg-white shadow rounded-lg p-6">
        ...
        <form action="{{ route('admin.games.spin.update') }}" method="POST" class="space-y-4">
            @csrf
            ...
            <table class="min-w-full divide-y divide-gray-200">
                <thead>
                    <tr>
                        ...
                        <th class="px-6 py-3 bg-gray-50 text-left text-xs font-medium text-gray-500 uppercase">Result Color</th>
                        <th class="px-6 py-3 bg-gray-50 text-left text-xs font-medium text-gray-500 uppercase">Multiplier</th>
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    ...
                    <tr>
                        ...
                        <td class="px-6 py-4">
                            <span class="px-2 py-1 rounded capitalize" style="background-color: {{ $game->result_color }}20; color: {{ $game->result_color }}">
                                {{ $game->result_color }}
                            </span>
                        </td>
                        <td class="px-6 py-4">{{ number_format($game->multiplier, 2) }}x</td>
                        <td class="px-6 py-4">₦{{ number_format($game->bets->sum('amount')) }}</td>
                        <td class="px-6 py-4">
                            <span class="px-2 py-1 text-xs rounded-full {{ $game->is_completed ? 'bg-green-100 text-green-800' : 'bg-blue-100 text-blue-800' }}">
