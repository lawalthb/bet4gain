    ...
    <div class="spin-game">
                ...
                <!-- <div class="recent-results">
                    ...
                    <div class="result-dots">
                        <div
                            ...
                            :title="`${result.color.toUpperCase()} - ${
                                result.multiplier
                            }x`"
                        >
                            {{ result.multiplier }}x
                ...
                <div class="stats-grid">
                    <div class="stat-box">
                        ...
                        <span class="stat-value"
                            >₦{{ isLoggedIn ? userBalance : demoBalance }}</span
                        >
                    </div>
                    <div class="stat-box">
        ...
        <div class="wheel-container">
            <img
                ...
                :style="{
                    transform: `rotate(${initialRotation + wheelRotation}deg)`,
                }"
                ref="wheel"
            />
        ...
        <div class="betting-panel">
            <div class="bet-options">
                <button
                    ...
                    :class="{
                        active: selectedColor === segment.color,
                        [segment.color]: true,
                    }"
                >
            ...
            <div class="bet-controls">
                ...
                />
                <button @click="placeBet" :disabled="!canBet || isSpinning">
                    Place Bet
                </button>
            </div>
        </div>
    ...
    setup() {
        ...
        const canBet = computed(() => {
            ...
            return (
                ...
                selectedColor.value &&
                betAmount.value > 0 &&
                betAmount.value <= demoBalance.value
            );
        });
        ...
        const spinWheel = (result) => {
            ...
            // Map colors to their exact degrees on the wheel image
            const colorDegrees = {
                green: 0,
                teal: 30,
                blue: 60,
            ...
            // Get exact degree for result color
            const targetDegree = colorDegrees[result];

            // Add spins and offset
            const spins = 5 * 360;
        ...
        const placeBet = async () => {
            ...
            try {
                if (!isLoggedIn.value) {
                    ...
                    const randomSegment =
                        segments.value[
                            Math.floor(Math.random() * segments.value.length)
                        ];
        ...
        const handleSpinResult = (result) => {
            // Calculate exact angle for result color
            const segmentAngle = 360 / segments.value.length
            const targetIndex = segments.value.findIndex(s => s.color === result)
            const targetAngle = 360 - (targetIndex * segmentAngle)

            // Set wheel rotation to point to result
        ...
        const updateStats = (data) => {
            ...
            stats.value.highestWin = Math.max(
                stats.value.highestWin,
                data.multiplier * betAmount.value
            );

            const greenGames = lastResults.value.filter(
                (r) => r.color === "green"
            ).length;
            stats.value.greenRate = (
                (greenGames / lastResults.value.length) *
                100
            ).toFixed(1);

            const totalMultiplier = lastResults.value.reduce(
                (sum, r) => sum + r.multiplier,
                0
            );
            stats.value.avgMultiplier = (
                totalMultiplier / lastResults.value.length
            ).toFixed(2);
        };
